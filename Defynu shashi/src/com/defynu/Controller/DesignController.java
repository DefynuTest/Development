...
public class DesignController extends HttpServlet {
 ...
 ArrayList<Shirt> sht = new ArrayList<Shirt>();
	...
	public ModelAndView designa(@RequestParam Map<String,String> reqPar ,HttpServletRequest request, HttpServletResponse response) {
		HttpSession session=request.getSession(false); 
		Shirt s1=new Shirt();
		s1=(Shirt) session.getAttribute("shirt");
		String body = reqPar.get("body");
		s1.setBody(body);
		String outercollar = reqPar.get("outercollar");
		s1.setOutercollar(outercollar);
	...
	public ModelAndView designb(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView designc(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
		HttpSession session=request.getSession(false); 
		Shirt s1=new Shirt();
		s1=(Shirt) session.getAttribute("shirt");
		...
		if(list.equalsIgnoreCase("type"))
		{
			...
			String id= outercollarid.substring(2,6);
	...
	public ModelAndView designic(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView designocu(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
		...
		if(list.equalsIgnoreCase("type"))
		{
			...
			String id= outercuffid.substring(2,6);
	...
	public ModelAndView designicu(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView designof(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView designbp(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	@RequestMapping(value = "/designbtn{someID}", method = RequestMethod.GET)
	public ModelAndView designbtn(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView designp(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) {
		...
		if(list.equalsIgnoreCase("type"))
		{
			...
			String id= pocketid.substring(2,6);
	...
	public ModelAndView Addtocart(@RequestParam Map<String, String> reqPar,
		...
		//	int qty = shirt.getQty() + 1;
			...
			Integer order_id = rand.nextInt((999 - 100) + 1) + 100;
			shirt.setOrder_id(order_id);
			...
			model.addObject("orderid", shirt.getOrder_id());
			...
			if (session.getAttribute("email") == null) {
				sht.add(shirt);
				session.setAttribute("neveruse", sht);
				int j = 0;
				int cart = 0;
				while (j < sht.size()) {
					cart = cart + sht.get(j).getQty();
		...
		*/
		...
		//	y = crt.AddtoCart(shirt, username.getUname());
			try {
				...
				rem.DeleteFromCart(request,shirt.getOrder_id());
				sht1 = crt.ShowCart(request);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
	...
	@RequestMapping(value = "/update{someID}", method = RequestMethod.POST)
	public ModelAndView update(@RequestParam Map<String, String> reqPar,
			HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
		
		
		String orderid = reqPar.get("orderno");
		Integer order_id=Integer.parseInt(orderid);
		
		
		if (session.getAttribute("email") == null) {
			int i=0;
			ArrayList<Shirt> chklist = new ArrayList<Shirt>();
			chklist=(ArrayList<Shirt>) session.getAttribute("neveruse");
			int size=chklist.size();
			for(i=0;i<size;i++){
		        int key=chklist.get(i).getOrder_id();
		        if(key==order_id)
		        {  
		        	chklist.remove(i);
		        	sht=chklist;
		...
		//	y = crt.AddtoCart(shirt, username.getUname());
			try {
				sht1 = crt.ShowCart(request);
				Remove rem= new Remove();
				rem.DeleteFromCart(request,order_id);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
