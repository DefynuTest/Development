...
public class LoginController extends HttpServlet{
	...
	public ModelAndView loginsucess(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("jjjjjjj"+chklist);
		
		sht1 = crt.ShowCart(request);
		if(chklist!=null)
		{
		for(int c=0;c<chklist.size();c++)
		{
			chklist.get(c).setNo(sht1.size() + 1);
		
		y = crt.AddtoCart(chklist.get(c), request);
		MeasurementDao measurement = new MeasurementDao();
		y=measurement.AddShirtDetails(request,chklist.get(c));
		y= measurement.Addmeasurenet(request,chklist.get(c));
		}

		////Main crt= new Main();
		ArrayList<Shirt> shtnew = new ArrayList<Shirt>();
		shtnew=crt.ShowCart(request);
		System.out.println("sht1 ka size" + shtnew.size());
		int j=0;
		int cart =0;
		while(j<shtnew.size())
		{
			cart=cart+ shtnew.get(j).getQty();
			j++;
		}
		...
		else
		{
			...
			model = new ModelAndView("hello");
			if(session.getAttribute("neveruse")!=null)
			{
			sht=(ArrayList<Shirt>) session.getAttribute("neveruse");
			System.out.println("sht ka size" + sht.size());
			int j=0;
			int cart =0;
			while(j<sht.size())
			{
	...
	public ModelAndView register(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response ) throws IOException {
		...
		HttpSession session=request.getSession(); 
		User user= new User(email,password, new Long(number));
		...
		if(!result){
			...
			session.setAttribute("newPhoneNumber",number);
	...
	public ModelAndView registerForm(){
		...
		public ModelAndView verifyNewUserOTP(@RequestParam Map<String,String> reqPar,HttpServletRequest request, HttpServletResponse response) throws Exception{
			...
			if(result){
				String email = session.getAttribute("newEmail").toString();
				String password = session.getAttribute("newPassword").toString();
				long phoneNumber = new Long(session.getAttribute("newPhoneNumber").toString());
